...
import { timed } from "@/server/lib/query-timing";
...
export async function getDecksWithStats(userId: string) {
  return timed("getDecksWithStats", () =>
    db
      .select({
        id: decks.id,
        name: decks.name,
...
export async function getDueStudyCards(
  ...
  const rows = await timed("getDueStudyCards", () =>
    db
      .select({
        id: cards.id,
        deckId: cards.deckId,
...
export async function getUserCard(cardId: string, userId: string) {
  const [row] = await timed("getUserCard", () =>
    db
      .select({ card: cards })
      .from(cards)
      .innerJoin(decks, eq(cards.deckId, decks.id))
...
export async function getUserDeck(deckId: string, userId: string) {
  const [deck] = await timed("getUserDeck", () =>
    db
      .select()
      .from(decks)
